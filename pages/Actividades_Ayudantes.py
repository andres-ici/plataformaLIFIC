...
#user
email: str = st.secrets["userEmailSupabase"]
password: str = st.secrets["userPasswordSupabase"]
...
def get_ayudantes(access_token):
    print('SELECT * from ayudantes')
    supabase.postgrest.auth(access_token)
    response = supabase.table('ayudantes').select("nombre, asignatura").execute()
...
def get_docentes(access_token):
    print('SELECT * from docentes')
    supabase.postgrest.auth(access_token)
    response = supabase.table('docentes').select("nombre, asignatura").execute()
...
def main():
    ...
    st.write(f'Quiere consultar las horas de {ayudante}')
